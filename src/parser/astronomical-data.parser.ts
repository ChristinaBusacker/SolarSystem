...
export class AstronomicalDataParser {
    ...
    public static parse(raw: AstronomicalRawData): AstronomicalDataset {
        ...
        const orbitalSpeed = raw.isOrbiting
            ? this.periodDaysToRadPerSecond(raw.orbitalPeriodDays!)
            : 0;
    ...
    private static validateRaw(raw: AstronomicalRawData): void {
        ...
        if (raw.apoapsisKm < raw.periapsisKm) {
            throw new Error(
                `AstronomicalRawData.apoapsisKm must be >= periapsisKm for "${raw.name}".`
            );
        }
        ...
        if (
            ...
            throw new Error(
                `ringOuterRadiusKm must be > ringInnerRadiusKm for "${raw.name}".`
            );
        }
    }
}