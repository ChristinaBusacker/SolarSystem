...
export const astronomicalDisplacementShader: ShaderDefinition = {
  vertexShader: `
...
void main() {
  ...
  // Zentriert um 0.0 (statt nur nach außen zu schieben)
  float displacement = (h - 0.5) * displacementHeight;

  // Entlang der lokalen Normalen verschieben
  vec3 displacedPosition = position + normalize(normal) * displacement;

  // World space normal (wie in deinem bisherigen Shader-Konzept)
  ...
  void main() {
    ...
    vec4 finalColor = vec4(dayColor.rgb * daynight * lightEffect * shadow, dayColor.a);
